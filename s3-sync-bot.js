...
async function listS3Objects(startAfter = null) {
    // Parse the last processed key to get date and hour
    // Format: node_fills_by_block/hourly/YYYYMMDD/H.lz4
    let startDate = null;
    let startHour = -1;
    ...
    // Generate date range: from startDate to today + 1 day
    ...
    while (currentDate <= endDateStr) {
        try {
            ...
            const filtered = contents
                ...
                .sort((a, b) => {
                    // Sort by date then hour numerically
...
async function runSync() {
    ...
    // DISABLED - using separate populate script instead
    // if (totalFills > 0) {
    //     await updateLeaderboardStats(newAddresses);
    // }
    
    // If more files to process, run again immediately
...
// Wrapper to prevent overlapping syncs
...
function connectLiquidationWs() {
    ...
    try {
        ...
        liqWs.on('open', () => {
            liqWs.send(JSON.stringify({ method: 'subscribe', subscription: { type: 'allLiquidations' } }));
            liqWsRetries = 0;
        ...
        liqWs.on('message', (raw) => {
            try {
                ...
                if (msg.channel === 'allLiquidations' && msg.data) {
                    ...
                    console.log(`🔴 HIP-3 Liq: ${coin} $${ntlValue.toFixed(2)}`);
        ...
        // FIX: Respond to server pings to keep connection alive
        liqWs.on('ping', (data) => {
            liqWs.pong(data);
        });

        liqWs.on('close', (code) => {
            console.log(`⚠️ Liquidation WebSocket closed (code: ${code}), reconnecting...`);
            liqWsRetries++;
            const delay = Math.min(5000 * liqWsRetries, 30000);
...
async function flushLiqBuffer() {
    ...
    try {
        ...
        if (liqEventsBuffer.length > 5000) {
            liqEventsBuffer = liqEventsBuffer.slice(-5000); // Prevent memory leak
        }
    }
}
...
function startLiquidationTracker() {
    ...
    // FIX: Keepalive ping every 20s (more aggressive)
    setInterval(() => {
    ...
    console.log('🔴 Liquidation tracker started (flushing every 10s)');
}
...
async function start() {
    ...
    server.listen(PORT, () => {
        ...
        // Reliable interval - runs every hour regardless of setTimeout issues
        setInterval(() => {
            console.log('\n⏰ Scheduled hourly sync triggered');
            safeSyncRun();
        }, SYNC_INTERVAL_MS);
        
        // Self-ping keep-alive every 5 minutes to prevent Koyeb sleep
        setInterval(() => {
