...
async function updateTraderStats(fills) {
    ...
    for (const addr of addresses) {
        ...
        // Parse existing markets (structure: {coin: {volume, fees, pnl, trades}})
        // This is correct - we add new values to existing totals
        ...
        // IMPORTANT: pnl_history stores BOTH daily_pnl (for calculations) and pnl (cumulative, for display)
        // Structure: [{date, daily_pnl, pnl}]
        let pnlHistory = typeof existingDetail.pnl_history === 'string'
            ? JSON.parse(existingDetail.pnl_history || '[]')
            : (existingDetail.pnl_history || []);
        
        // ═══════════════════════════════════════════════════════════════════
        // CRITICAL FIX: Load DAILY PnL values, NOT cumulative values
        // The old bug was: pnlByDate[p.date] = p.pnl (cumulative) 
        // Then we re-cumulated, causing exponential growth
        // ═══════════════════════════════════════════════════════════════════
        const pnlByDate = {};
        for (const p of pnlHistory) {
            // Read daily_pnl (the PnL for that specific day only)
            // If daily_pnl doesn't exist (old corrupted data), we can't recover - use 0
            // After a reset, all new data will have daily_pnl
            if (p.daily_pnl !== undefined) {
                pnlByDate[p.date] = p.daily_pnl;
            }
            // Do NOT use p.pnl here - that's cumulative and would cause the bug
        }
        ...
        for (const fill of addrFills) {
            ...
            // Update market details (this part is correct - adding to totals)
            ...
            // Update daily PnL - add to the DAILY value for this date
            if (fill.trade_time) {
                const date = new Date(fill.trade_time).toISOString().split('T')[0];
                if (!pnlByDate[date]) pnlByDate[date] = 0;
                pnlByDate[date] += fill.closed_pnl; // Add to daily total
            }
        }
        
        // ═══════════════════════════════════════════════════════════════════
        // Convert pnlByDate to array with BOTH daily_pnl and cumulative pnl
        // - daily_pnl: used by the bot to correctly accumulate on next sync
        // - pnl: cumulative value for frontend display
        // ═══════════════════════════════════════════════════════════════════
        ...
        const pnlHistoryFinal = sortedDates.map(date => {
            ...
            return { 
                date: date,
                daily_pnl: dailyPnl,  // PnL for THIS day only (for bot calculations)
                pnl: cumulative       // Running total (for frontend display)
            };
        });
...
async function runSync() {
    ...
    for (const obj of filesToProcess) {
        ...
        if (hip3Fills.length > 0) {
            ...
            // Update stats immediately per file (avoid memory accumulation)
