...
async function downloadAndParse(key) {
    try {
        ...
        // Skip files larger than 50MB to avoid OOM
                    ...
                    if (line.trim()) {
                        try {
                            ...
                            if (Array.isArray(parsed)) {
                                totalFills += parsed.length;
                                for (const fill of parsed) {
                                    if (fill.coin && fill.coin.includes(':')) {
                                        data.push(fill);
                                        hip3Fills++;
                                    }
                                }
                                ...
                                totalFills++;
                                if (parsed.coin.includes(':')) {
                                    data.push(parsed);
                                    hip3Fills++;
                                }
                            } else if (parsed.fills && Array.isArray(parsed.fills)) {
                                // Maybe fills are nested
                                totalFills += parsed.fills.length;
...
async function runSync() {
    ...
    // Process only 10 files per sync to avoid OOM
    const filesToProcess = objects.slice(0, 10);
    ...
    // Update trader stats only if we saved fills
    if (totalFills > 0) {
    ...
    if (objects.length > 10) {
        console.log(`\n⏰ More files pending, next sync in 30s...`);
