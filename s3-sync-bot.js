...
// ═══════════════════════════════════════════════════════════════════════════
// Update hip3_daily_stats - only count side='B' to avoid double counting
// ═══════════════════════════════════════════════════════════════════════════

async function updateDailyStats(fills) {
    if (fills.length === 0) return;
    
    // Only count buys to avoid double counting (each trade has buyer + seller)
    const buyFills = fills.filter(f => f.side === 'B');
    if (buyFills.length === 0) return;
    
    // Group by date and deployer
    const dailyStats = {};
    
    for (const fill of buyFills) {
        if (!fill.trade_time) continue;
        const date = new Date(fill.trade_time).toISOString().split('T')[0];
        const deployer = fill.coin.split(':')[0];
        const key = `${date}|${deployer}`;
        
        if (!dailyStats[key]) {
            dailyStats[key] = { date, deployer, volume: 0, trades_count: 0 };
        }
        dailyStats[key].volume += fill.px * fill.sz;
        dailyStats[key].trades_count += 1;
    }
    
    // Upsert to hip3_daily_stats
    const records = Object.values(dailyStats);
    ...
    for (const record of records) {
        try {
            ...
            // If conflict, update by adding
            if (e.response?.status === 409) {
                try {
                    const existing = await axios.get(
                        `${SUPABASE_URL}/rest/v1/hip3_daily_stats?date=eq.${record.date}&deployer=eq.${record.deployer}`,
                        { headers: sbHeaders() }
                    );
                    if (existing.data?.[0]) {
                        await axios.patch(
                            `${SUPABASE_URL}/rest/v1/hip3_daily_stats?date=eq.${record.date}&deployer=eq.${record.deployer}`,
                            {
                                volume: existing.data[0].volume + record.volume,
                                trades_count: existing.data[0].trades_count + record.trades_count
                            },
                            { headers: sbHeaders() }
                        );
                    }
                } catch (e2) {
                    console.log(`    Daily stats update error: ${e2.message}`);
                }
            }
        }
    }
    
    console.log(`    📈 Updated daily stats for ${records.length} date/deployer combos`);
}
