...
// ═══════════════════════════════════════════════════════════════════════════
// Update hip3_daily_stats - recalculate from hip3_fills for affected dates
// ═══════════════════════════════════════════════════════════════════════════

async function updateDailyStats(fills) {
    if (fills.length === 0) return;
    
    // Get unique dates from fills
    const dates = new Set();
    for (const fill of fills) {
        if (fill.trade_time) {
            const date = new Date(fill.trade_time).toISOString().split('T')[0];
    ...
    console.log(`    📈 Recalculating daily stats for ${dates.size} date(s)...`);
    
    // For each date, delete and recalculate from hip3_fills
    for (const date of dates) {
        try {
            // Delete existing records for this date
            await axios.delete(
                `${SUPABASE_URL}/rest/v1/hip3_daily_stats?date=eq.${date}`,
                { headers: sbHeaders() }
            );
            
            // Calculate start/end timestamps for this date (UTC)
            const startOfDay = new Date(date + 'T00:00:00Z').getTime();
            const endOfDay = startOfDay + 86400000; // +24h
            
            // Query hip3_fills for this date, grouped by deployer, only side='B'
            const response = await axios.get(
                `${SUPABASE_URL}/rest/v1/rpc/get_daily_stats_for_date`,
                { 
                    headers: sbHeaders(),
                    params: { start_ts: startOfDay, end_ts: endOfDay }
                }
            );
            ...
            if (response.data && response.data.length > 0) {
                for (const row of response.data) {
                    await axios.post(
                        `${SUPABASE_URL}/rest/v1/hip3_daily_stats`,
                        { date, deployer: row.deployer, volume: row.volume, trades_count: row.trades_count },
            ...
            if (e.response?.status === 404 || e.message.includes('get_daily_stats_for_date')) {
                await recalculateDailyStatsFallback(date);
            } else {
                console.log(`    Daily stats error for ${date}: ${e.message}`);
            }
        }
    }
}

async function recalculateDailyStatsFallback(date) {
    try {
        const startOfDay = new Date(date + 'T00:00:00Z').getTime();
        const endOfDay = startOfDay + 86400000;
        
        // Fetch all fills for this date (paginated)
        const fills = [];
        let offset = 0;
        const limit = 1000;
        ...
        while (true) {
            const res = await axios.get(
                `${SUPABASE_URL}/rest/v1/hip3_fills?trade_time=gte.${startOfDay}&trade_time=lt.${endOfDay}&side=eq.B&select=coin,px,sz&limit=${limit}&offset=${offset}`,
                { headers: sbHeaders() }
            );
        ...
        const stats = {};
        for (const fill of fills) {
            const deployer = fill.coin.split(':')[0];
            if (!stats[deployer]) {
                stats[deployer] = { volume: 0, trades_count: 0 };
            }
            stats[deployer].volume += fill.px * fill.sz;
            stats[deployer].trades_count += 1;
        }
        
        // Insert
        for (const [deployer, data] of Object.entries(stats)) {
            await axios.post(
                `${SUPABASE_URL}/rest/v1/hip3_daily_stats`,
                { date, deployer, volume: data.volume, trades_count: data.trades_count },
                { headers: { ...sbHeaders(), 'Prefer': 'return=minimal' } }
            );
        }
        
        console.log(`    ✅ Recalculated ${date}: ${Object.keys(stats).length} deployers, ${fills.length} fills`);
    } catch (e) {
        console.log(`    Fallback error for ${date}: ${e.message}`);
    }
}
...
async function runSync() {
    ...
    for (const obj of filesToProcess) {
        ...
        if (hip3Fills.length > 0) {
            ...
            // Update daily stats (only counts side='B' to avoid double counting)
            await updateDailyStats(hip3Fills);
        }
