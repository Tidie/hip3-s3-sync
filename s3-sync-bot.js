...
async function updateTraderStats(fills) {
    ...
    for (const addr of addresses) {
        ...
        // Parse existing pnl_history (daily values)
        ...
        for (const p of pnlHistory) {
            pnlByDate[p.date] = p.pnl; // daily pnl
        }
        ...
        // Convert pnlByDate back to array (cumulative)
...
async function runSync() {
    ...
    for (const obj of filesToProcess) {
        ...
        if (hip3Fills.length > 0) {
            ...
            // Collect for stats update (avoid spread for large arrays)
            for (const fill of hip3Fills) {
                allFillsThisSync.push(fill);
            }
        }
    ...
    // Update trader stats after processing all files
    if (allFillsThisSync.length > 0) {
        await updateTraderStats(allFillsThisSync);
    }
